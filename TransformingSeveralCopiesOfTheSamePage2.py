...
for pageNo in range(len(reader.pages)):  # Loop through all source pages
    ...
    # Get width in points
    width_in_points = (
        sourcepage.mediabox.upper_right[0] - sourcepage.mediabox.upper_left[0]
    )
...
for i in range(1, 4):
    fold_lines.append(
        (i * page_width / 4, 0, i * page_width / 4, page_height)
    )  # Vertical lines

for i in range(1, 2):
    fold_lines.append(
        (0, i * page_height / 2, page_width, i * page_height / 2)
    )  # Horizontal line

# Add fold lines as annotations
for start_x, start_y, end_x, end_y in fold_lines:
    annotation = Line(
        rect=(start_x, start_y, end_x, end_y), p1=(start_x, start_y), p2=(end_x, end_y)
    )
...
output_pdf = "output_{}.pdf".format(current_date + "_" + current_time)
